...
TARGET_RECOVERY_FSTAB = device/fsl/pico_8m/fstab.freescale
# build for ext4
PRODUCT_COPY_FILES +=	\
	device/fsl/pico_8m/fstab.freescale:root/fstab.freescale
else
TARGET_RECOVERY_FSTAB = device/fsl/pico_8m/fstab-f2fs.freescale
# build for f2fs
PRODUCT_COPY_FILES +=	\
	device/fsl/pico_8m/fstab-f2fs.freescale:root/fstab.freescale
...
ADDITION_BPT_PARTITION = partition-table-7GB:device/fsl/common/partition/device-partitions-7GB-ab.bpt \
             ...
             partition-table-14GB:device/fsl/common/partition/device-partitions-13GB-ab.bpt \
             partition-table-28GB:device/fsl/common/partition/device-partitions-28GB-ab.bpt

# Vendor Interface manifest and compatibility
DEVICE_MANIFEST_FILE := device/fsl/pico_8m/manifest.xml
DEVICE_MATRIX_FILE := device/fsl/pico_8m/compatibility_matrix.xml

TARGET_BOOTLOADER_BOARD_NAME := PICO-8M
...
PRODUCT_COPY_FILES +=	\
       device/fsl/pico_8m/ueventd.freescale.rc:root/ueventd.freescale.rc
...
BOARD_KERNEL_CMDLINE := console=ttymxc0,115200 earlycon=imxuart,0x30860000,115200 init=/init androidboot.console=ttymxc0 consoleblank=0 androidboot.hardware=freescale androidboot.fbTileSupport=enable androidboot.primary_display=imx-drm firmware_class.path=/vendor/firmware androidboot.selinux=permissive
...
BOARD_SEPOLICY_DIRS := \
       ...
       device/fsl/pico_8m/sepolicy

ifeq ($(PRODUCT_IMX_DRM),true)
BOARD_SEPOLICY_DIRS += \
       device/fsl/imx8/sepolicy_drm \
       device/fsl/pico_8m/sepolicy_drm
endif

# Vendor seccomp policy files for media components:
PRODUCT_COPY_FILES += \
       device/fsl/pico_8m/seccomp/mediacodec-seccomp.policy:vendor/etc/seccomp_policy/mediacodec.policy \
       device/fsl/pico_8m/seccomp/mediaextractor-seccomp.policy:vendor/etc/seccomp_policy/mediaextractor.policy

PRODUCT_COPY_FILES += \
       device/fsl/pico_8m/app_whitelist.xml:system/etc/sysconfig/app_whitelist.xml
